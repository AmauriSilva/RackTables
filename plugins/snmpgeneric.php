...
//	sg_var_dump_html($sysObjectID['port']);
	...
	foreach($ifsnmp->ipaddress as $ifindex => $ipaddresses) {
		//echo "<br> - DEBUG: ipspace $ipaddresses[0]<br>";
	...
	if(!empty($ipspace)) {
		...
		foreach($ipspace as $prefix => $ipspace) {
			$netcreatecheckbox = '<b style="background-color:#00ff00">'
				...
				.$i.']" value="'.$ipspace['addrtype'].'"'.($ipspace['checked'] ? '' : '').'></b>';
	...
	echo '<tr><td colspan="11"></td>
		...
		<td><input type="checkbox" id="label" onclick="setchecked(this.id);"></td>
		<td><input type="checkbox" id="mac" onclick="setchecked(this.id);"></td>
			...
			foreach($ipaddresses as $ipaddr => $value) {
				...
				if(empty($address['allocs'])) {
					...
					$ipaddrhref = makeHref(array('page'=>'object',
									 ...
									 'tab'=>'default',
									 'object_id' => $ipobject_id, 'hl_ip' => $ipaddr));
				}
				/* reserved addresses */
...
class ifSNMP implements Iterator {
	...
	//		sg_var_dump_html($ipAddressPrefix);
		...
		if($ipv6interfaces)
		{
			...
			//var_dump($this->snmpdevice);
			//$ipv6addraddress =  $this->snmpdevice->walk('ipAddressIfIndex.ipv6');
			snmp_set_oid_output_format(SNMP_OID_OUTPUT_NUMERIC);
			$ipv6AddrPfxLength = $this->snmpdevice->walk('ipv6AddrPfxLength');
			if(!empty($ipv6AddrPfxLength)) {
				foreach($ipv6AddrPfxLength as $oid => $maskbits) {
					$dec = explode(".", $oid);
					$addr = sprintf("%02x",$dec[13]).
						sprintf("%02x",$dec[14]).":".
						sprintf("%02x",$dec[15]).
						sprintf("%02x",$dec[16]).":".
						sprintf("%02x",$dec[17]).
						...
						sprintf("%02x",$dec[27]).
						sprintf("%02x",$dec[28]);
					$ifindex =  array_search($dec[12],$this->ifTable['ifIndex']);
					if($ifindex === false)
						continue;
					$addr_bin = ip_parse($addr);
			...
			//$ipv6addraddress =  array (
			//	".1.3.6.1.2.1.55.1.8.1.2.526.32.1.18.240.5.0.255.28.0.0.0.0.0.0.0.2" => "2001:12f0:0500:ff1c:0000:0000:0000:0002"
			//);
			if(!empty($ipv6addraddress)) {
				$ipv6addrpfxlength =  $this->snmpdevice->walk('ipv6AddrPfxLength');
			//	var_dump($ipv6addrpfxlength);
