...
function fdry5ScanTopLevel (&$work, $line)
{
	...
	switch (TRUE)
	{
	case (preg_match ('@^vlan (\d+)( name .+)$@', $line, $matches)):
		if (!array_key_exists ($matches[1], $work['vlanlist']))
			$work['vlanlist'][] = $matches[1];
		$work['current'] = array ('vlan_id' => $matches[1]);
		return 'fdry5-get8021q-readvlan';
		break;
	case (preg_match ('@^interface ethernet (\d+/\d+)$@', $line, $matches)):
		$port_name = 'e' . $matches[1];
		$work['current'] = array ('port_name' => $port_name);
		$work['portconfig'][$port_name][] = array ('type' => 'line-header', 'line' => $line);
		return 'fdry5-get8021q-readport';
		break;
	case (preg_match ('@^interface ve (\d+)$@', $line, $matches)):
		$port_name = 've ' . $matches[1];
		$work['current'] = array ('port_name' => $port_name);
		$work['portconfig'][$port_name][] = array ('type' => 'line-header', 'line' => $line);
		return 'fdry5-get8021q-readport';
		break;
...
function fdry5PickVLANSubcommand (&$work, $line)
{
	...
	case (preg_match ('@^ untagged (.+)$@', $line, $matches)):
		...
		break;
	case (preg_match ('@^ router-interface (.+)$@', $line, $matches)):
		$port_name = $matches[1];
                        if (array_key_exists ($port_name, $work['portdata']))
                                $work['portdata'][$port_name]['allowed'][] = $work['current']['vlan_id'];
                        else
...
function jun13TranslatePushQueue ($dummy_object_id, $queue, $vlan_names)
{
	$ret = '';
...
function xos12Read8021QConfig ($input)
{
	...
	foreach (explode ("\n", $input) as $line)
	{
		...
		switch (TRUE)
		{
		case (preg_match ('/^create vlan "(VLAN_.+)"$/', $line, $matches)):
			if (!preg_match ('/^VLAN_\d+.+$/', $matches[1]))
				throw new RTGatewayError ('unsupported VLAN name ' . $matches[1]);
			break;
		case (preg_match ('/^configure vlan (\S+) tag (\d+)$/', $line, $matches)):
			if (strtolower ($matches[1]) == 'default')
				throw new RTGatewayError ('default VLAN tag must be 1');
			if (substr($matches[1], 0, 9) != 'VLAN_' . sprintf("%04d", $matches[2]))
				throw new RTGatewayError ("VLAN name ${matches[1]} does not match its tag ${matches[2]} Please, use VLAN_0000_Vlan_Name");
			$ret['vlanlist'][] = $matches[2];
			break;
		case (preg_match ('/^configure vlan (\S+) add ports (.+) (tagged|untagged) */', $line, $matches)):
			$submatch = array();
			#var_dump($matches[1]);
			if ($matches[1] == 'Loopback0'){
				continue;
			}elseif ($matches[1] == 'Default'){
				#$matches[1] = 'VLAN_1';
				$vlan_id = 1;
			}elseif (preg_match ('/^VLAN_(\d+).+$/', $matches[1], $submatch)){
				$vlan_id = intval($submatch[1]);
			}else
				throw new RTGatewayError ('unsupported VLAN name ' . $matches[1] . 'Please, use VLAN_0000_Vlan_Name');
			#$vlan_id = $submatch[1];
...
function jun13Read8021QConfig ($input)
{
	$ret = constructRunning8021QConfig();
	$ret['vlannames'][VLAN_DFL_ID] = 'default';
	...
	while (count ($lines))
	{
		...
		if (FALSE !== strpos ($line, '# END OF VLAN LIST'))
			break;
		if (preg_match ('/^VLAN Name: (\S+)/', $line, $m))
		{
			$ret['vlannames'] = $m[1];
		}
		if (preg_match ('/^Tag: (\d+)/', $line, $m))
		{
			$vlans[$ret['vlannames']] = $m[1];
		}
	}
	$ret['vlanlist'] = array_values	($vlans);
	// get config groups list - throw an exception if a group contains ether-switching config
	...
	while (count ($lines))
	{
		...
		if (preg_match ('/# END OF CONFIG|^(interface-range )?^\s+unit\s(\d+)\s{$/', $line, $m))
		{ // found interface section opening, or end-of-file
			//if (isset ($current['name']) and $current['is_ethernet'])
			if (isset ($current['name']))
			{
				...
				{
					if ($current['config']['mode'] == 'access')
						$current['config']['allowed'] = array (0);
					else
						$current['config']['allowed'] = array();
				}
				if (
			...
			if (isset ($m[2]))
			{ // new interface section begins
				#var_dump ($interface.'.'.$m[2], '</br>');
				$current['is_ethernet'] = FALSE;
				$current['is_range'] = ! empty ($m[1]);
				$current['name'] = $interface.'.'.$m[2];
				$current['config'] = array (
					'mode' => NULL,
					'allowed' => NULL,
					'native' => NULL,
		...
		elseif (isset ($current['indent']) and $line == $current['indent'] . '}')
			$current['indent'] = NULL;
		elseif ($current['is_ethernet'] and isset ($current['indent']))
		#elseif ($current['is_ethernet'])
		{
			$line_class = 'line-8021q';
			if (preg_match ('/^\s+interface-mode (trunk|access);/', $line, $m))
				$current['config']['mode'] = $m[1];
			elseif (preg_match ('/^\s+members \[?(.*)\]?;$/', $line, $m))
			{
				$members = array();
		...
		elseif (preg_match ('/^\s+vlan-id (\d+);/', $line, $m))
		{
				$line_class = 'line-8021q';
                                $current['config']['allowed'] =  array($m[1]);
		}
		if (isset ($current['name']))
		{
			if ($line == '    }')
...
function jun13ReadInterfaceStatus ($input)
{
	$result = array();
	$state = 'headerSearch';
	foreach (explode ("\n", $input) as $line)
...
function jun13SpotConfigText ($input)
{
	return preg_replace ('/.*?^## Last commit: \N*\n(.*)^\S+@\S+>\s*\Z/sm', '$1', $input, 1);
}
...
function jun13ReadLLDPStatus ($input)
{
        $ret = array();
		...
		#var_dump ($line, '</br>');
                ...
                elseif ($line == "")
                        $lldp_mode = FALSE;
                elseif ($lldp_mode && preg_match ('/^(\S+).*\s+([0-9a-f:]{17})\s+(.*)\s.*\s+(\S+)\s*$/', $line, $m))
                        $ret[shortenIfName ($m[1])][] = array
                        (
                                'port' => $m[3],
                                'device' => $m[4],
